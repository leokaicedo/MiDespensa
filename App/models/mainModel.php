...
class mainModel
{
	...
	/*---------- Funcion seleccionar datos ----------*/
	public function seleccionarDatos($tipo, $tabla, $campo = null, $id = null)
	{
		// Limpiar las variables de entrada
		$tipo = $this->limpiarCadena($tipo);
		$tabla = $this->limpiarCadena($tabla);

		// Verificamos el tipo de selección
		if ($tipo == "Unico") {
			...
			// Seleccionar un campo específico de todos los registros
			$campo = $this->limpiarCadena($campo);
			$sql = $this->conectar()->prepare("SELECT $campo FROM $tabla");
		} elseif ($tipo == "Todos") {
			...
			// Si el tipo no es válido, devolvemos null o se puede manejar un error
			return null;
		}
	...
	private $model; // Definición de la propiedad
	...
	public function eliminarRegistro($tabla, $campo, $id)
	{
