...
class userController extends mainModel
{
	...
	public function eliminarUsuarioControlador() {
        ...
        // Verificando usuario
        $datos = $this->model->ejecutarConsulta("SELECT * FROM usuario WHERE usuario_id='$id'");
        if ($datos->rowCount() <= 0) {
            $alerta = [
                "tipo" => "simple",
                "titulo" => "Ocurrió un error inesperado",
        ...
        $eliminarUsuario = $this->model->eliminarRegistro("usuario", "usuario_id", $id);

        if ($eliminarUsuario->rowCount() == 1) {
            // Lógica para eliminar la foto del usuario
            $alerta = [
                "tipo" => "recargar",
                "titulo" => "Usuario eliminado",
                "texto" => "El usuario " . $datos['usuario_nombre'] . " " . $datos['usuario_apellido'] . " ha sido eliminado del sistema correctamente",
